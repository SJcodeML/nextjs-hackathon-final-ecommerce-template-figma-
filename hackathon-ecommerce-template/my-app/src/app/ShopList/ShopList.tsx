...
export default function ShopListComponent (){
    return (
        <div className="bg-white">
                ...
                <div className='max-w-7xl mx-auto md:ml-52'>  
                    <h1 className="text-3xl md:text-4xl leading-10 text-[#101750] font-bold">Shop List</h1>  
                    <div className='flex  mt-5 mb-20 space-x-2'>  
                        <h3 className="text-[#3f509e]">Home . </h3>  
                        <h3 className="text-[#3f509e]">Pages . </h3>  
                        <h3 className="text-[#3f509e]">Shop List</h3>  
                    </div>  
                </div>  
       ...
       <div className="flex flex-wrap justify-center mx-auto  gap-4  mb-20 max-w-screen-lg">  
        {shoplistdata.map((item) => (  
        <div className="w-full sm:w-80 md:w-[52rem] lg:w-[90rem] m-2 bg-white flex flex-col md:flex-row shadow-md px-4 mb-4">  
            {/* Image Container */}  
            <div className="flex justify-center items-center mt-4 w-full h-48 overflow-hidden">  
                <Image  
                    src={item.image}  
            ...
            {/* Card Details */}  
            <div className="p-4 flex flex-col leading-6 my-2 w-full">  
                <div className="flex flex-row justify-between items-start">  
                    <h3 className="text-[#151875] text-[1rem] font-bold">{item.name}</h3>   
                    <div className="flex gap-1 justify-center items-center">  
                        <span className="rounded-full bg-[#de9034] text-lg w-2 h-2"></span>  
                        <span className="rounded-full bg-[#e60584] text-lg w-2 h-2"></span>  
                        <span className="rounded-full bg-[#5e37ff] text-lg w-2 h-2"></span>  
                ...
                <div className="flex flex-col justify-around my-3">   
                    <div className="flex items-center gap-3 ">  
                        <span className="text-[#151875] text-[0.875rem]">${item.price.toFixed(2)}</span>  
                        {item.oldPrice && (  
                            <span className="text-[#ba3131] text-[0.875rem] line-through">${item.oldPrice.toFixed(2)}</span>  
                        )}   
                        <div className="flex ">  
                            <Stars src={shoplist} />  
                        </div>  
                    </div>   
                    <h3 className="text-[#9295aa] text-[0.75rem] leading-8 font-[400]">{item.description}</h3>  
                    <div className="flex gap-3 mt-2">  
                        <span className="rounded-full bg-white w-7 h-7 flex items-center justify-center">  
                            <FontAwesomeIcon icon={faCartShopping} className="text-[#464a6e]" />  
                        </span>  
                        <span className="rounded-full bg-white w-7 h-7 flex items-center justify-center">  
                            <FontAwesomeIcon icon={faHeart} className="text-[#464a6e]" />  
                        </span>  
                        <span className="rounded-full bg-white w-7 h-7 flex items-center justify-center">  
                            <FontAwesomeIcon icon={faMagnifyingGlassPlus} className="text-[#464a6e]"/>  
                        </span>   
                    </div>   
                </div>  
            </div>   
